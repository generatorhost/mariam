...
def test_runtime_completion_report_summarizes_project_readiness() -> None:
    ...
    assert report["completion_percent"] < 70
    ...
    assert {area["name"] for area in report["areas"]} >= {
        ...
        "DNA and plugin evolution",
        "Model provider ecosystem",
        "Knowledge and vector system",
    }
    assert "not a finished autonomous enterprise" in report["summary"]
...
def test_runtime_implementation_roadmap_orders_next_work() -> None:
    ...
    assert roadmap["items"][0]["area"] == "Agent and workflow runtime"
    assert roadmap["items"][0]["priority"] == "medium"
    assert roadmap["items"][1]["area"] == "Knowledge and vector system"
...
def test_runtime_implementation_roadmap_can_be_exported_as_review_package() -> None:
    ...
    assert export_package["package_manifest"]["first_priority_area"] == "Agent and workflow runtime"
